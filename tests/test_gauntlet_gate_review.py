...
two-sided demonstration matrix.
  ...
  * llm_review: strict approve->pass, request_changes->fail; raw req/resp digested; measurement
  only.
  * gate: outcome DERIVED from result_kind; gate_outcome comes from the REAL account() (the fake
    never re-encodes it — a BlockingRefusal is run_verdict, not block_gate); binds the tree it ran.
...
_MD = "a" * 64
_PROMPT_HASH = "b" * 64
# result_kind -> the account() helper key (non_run splits into block/neutral; the fake picks block).
_ACCOUNT_KEY = {
...
def _review_client(verdict: str):  # noqa: ANN202
    def client(_view: Path, reviewer_lineage: str, prompt_hash: str) -> ReviewOutcome:
...
def test_llm_review_approve_is_pass(tmp_path: Path) -> None:
    ...
    obs = llm_review_stage(tmp_path, reviewer_lineage="gpt-y", review_prompt_hash=_PROMPT_HASH,
...
# ---- gate: outcome derivation + measured-digest binding (gate_outcome from real account()) ----

def _gate_runner(result_kind: str, *, admitted: str | None = None, measured: str | None = None):  # noqa: ANN202,E501
    def run(view: Path) -> GateMeasurement:
        return GateMeasurement(
            ...
            gate_outcome=real_gate_outcome(_ACCOUNT_KEY[result_kind]), admitted_outcome=admitted,
...
def test_gate_blocking_refusal_is_blocked(tmp_path: Path) -> None:
    (tmp_path / "m.py").write_text("x = 1\n")
    obs = gate_stage(tmp_path, gate_runner=_gate_runner("blocking_refusal"))
...
def test_gate_admitted_pass_and_fail(tmp_path: Path) -> None:
    (tmp_path / "m.py").write_text("x = 1\n")
    p = gate_stage(tmp_path, gate_runner=_gate_runner("admitted_run", admitted="pass"))
    f = gate_stage(tmp_path, gate_runner=_gate_runner("admitted_run", admitted="fail"))
...
def test_gate_infra_is_error(tmp_path: Path) -> None:
    (tmp_path / "m.py").write_text("x = 1\n")
    obs = gate_stage(tmp_path, gate_runner=_gate_runner("infrastructure_failure"))
